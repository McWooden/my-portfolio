...
export default function ProjectDetail() {
  ...
  return (
    <div className="pt-20 bg-bg-dark">
      ...
      {/* Header */}
      <section className="pt-[100px] pb-[60px] px-10 max-[810px]:pt-[60px] max-[810px]:px-5 max-w-[1200px] mx-auto flex flex-col gap-[30px] border-b border-border">
        <div className="flex flex-col gap-4">
          ...
          <p className="text-[1.35rem] text-text-secondary leading-[1.4] max-w-[700px]">
            {project.subtitle}
          </p>
        </div>

        {/* Meta block */}
        <div className="flex flex-col gap-[30px] mt-5 md:flex-row md:justify-between md:items-start">
          {/* Tags */}
          ...
          {/* Testimonial */}
          <div className="bg-bg-card border border-border p-[30px] rounded-[24px] max-w-[600px] flex flex-col gap-4">
          ...
          {project.images.map((img, idx) => (
            <div key={idx} className="flex flex-col gap-4 w-full">
              <div className="w-full rounded-[30px] overflow-hidden border border-border">
                <img src={img} alt={`${project.title} Detail ${idx + 1}`} className="w-full" />
              </div>
              <span className="font-mono text-[0.9rem] text-text-muted uppercase tracking-[0.05em] pl-[10px]">
                {project.imageCaps[idx] || 'Detail View'}
              </span>
            </div>
          ))}
        </div>
